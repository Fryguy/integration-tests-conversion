...
alias _conditions conditions
require_relative 'cfme/rest/gen_data'
include Cfme::Rest::Gen_data
alias _policies policies
...
class TestConditionsRESTAPI
  def conditions(request, appliance)
    num_conditions = 2
  ...
  def test_query_condition_attributes(conditions, soft_assert)
  ...
  def test_create_conditions(appliance, conditions)
    ...
    for condition in conditions
      ...
      raise unless record.description == condition.description
    end
  end
  def test_delete_conditions_from_detail(conditions, method)
  ...
  def test_delete_conditions_from_collection(conditions)
  ...
  def test_edit_conditions(conditions, appliance, from_detail)
    ...
    raise unless edited.size == num_conditions
    for (index, condition) in enumerate(conditions)
      record,__ = wait_for(lambda{|| appliance.rest_api.collections.conditions.find_by(description: new[index]["description"]) || false}, num_sec: 100, delay: 5, message: "Find a test condition")
      condition.reload()
...
class TestPoliciesRESTAPI
  def policies(request, appliance)
    num_policies = 2
    response = _policies(request, appliance, num: num_policies)
    assert_response(appliance)
    raise unless response.size == num_policies
    return response
  end
  def test_query_policy_attributes(policies, soft_assert)
  ...
  def test_create_policies(appliance, policies)
    ...
    for policy in policies
      ...
      raise unless record.description == policy.description
    end
  end
  def test_delete_policies_from_detail_post(policies)
  ...
  def test_delete_policies_from_detail_delete(policies)
  ...
  def test_delete_policies_from_collection(policies)
  ...
  def test_edit_policies(policies, appliance, from_detail)
    # Tests edit policies.
    # 
    #     Testing BZ 1435777
    # 
    ...
    raise unless edited.size == num_policies
    for (index, policy) in enumerate(policies)
      record,__ = wait_for(lambda{|| appliance.rest_api.collections.policies.find_by(description: new[index]["description"]) || false}, num_sec: 100, delay: 5, message: "Find a policy")
      policy.reload()
      raise unless (policy.description == edited[index].description) and (edited[index].description == record[0].description)
    end
  end
  def test_create_invalid_policies(appliance)
    # 
