...
pytestmark = [pytest.mark.meta(server_roles: ["+automate", "+smartstate", "+smartproxy"]), pytest.mark.tier(3), test_requirements.control, pytest.mark.provider([VMwareProvider], scope: "module"), pytest.mark.usefixtures("setup_provider_modscope")]
def policy_name()
...
def compliance_vm(configure_fleecing_modscope, provider, full_template_modscope)
  ...
  if provider.version == 6.5
    vm.create_on_provider(allow_skip: "default", host: conf.cfme_data["management_systems"][provider.key]["hosts"][0].name)
  else
...
def analysis_profile(appliance)
  ap = appliance.collections.analysis_profiles.instantiate(name: "default", description: "ap-desc", profile_type: appliance.collections.analysis_profiles.VM_TYPE, categories: ["Services", "User Accounts", "Software", "VM Configuration", "System"])
...
def test_check_package_presence(request, appliance, compliance_vm, analysis_profile)
  ...
  #       caseimportance: high
  #   
  condition = appliance.collections.conditions.create(VMCondition, fauxfactory.gen_alphanumeric(40, start: "Compliance testing condition "), expression: "fill_find(field=VM and Instance.Guest Applications : Name, skey=STARTS WITH, value=kernel, check=Check Count, ckey= = , cvalue=1)")
  request.addfinalizer(lambda{|| diaper(condition.delete)})
  policy = appliance.collections.policies.create(VMCompliancePolicy, fauxfactory.gen_alphanumeric(15, start: "Compliance "))
  request.addfinalizer(lambda{|| diaper(policy.delete)})
  policy.assign_conditions(condition)
  profile = appliance.collections.policy_profiles.create(fauxfactory.gen_alphanumeric(20, start: "Compliance PP "), policies: [policy])
  ...
  raise unless compliance_vm.compliant
end
def test_check_files(request, appliance, compliance_vm, analysis_profile)
  ...
  condition = appliance.collections.conditions.create(VMCondition, fauxfactory.gen_alphanumeric(40, start: "Compliance testing condition "), expression: "fill_find(VM and Instance.Files : Name, =, {}, Check Any, Contents, INCLUDES, {})".format(check_file_name, check_file_contents))
  request.addfinalizer(lambda{|| diaper(condition.delete)})
  policy = appliance.collections.policies.create(VMCompliancePolicy, fauxfactory.gen_alphanumeric(15, start: "Compliance "))
  request.addfinalizer(lambda{|| diaper(policy.delete)})
  policy.assign_conditions(condition)
...
def test_compliance_with_unconditional_policy(host, assign_policy_for_testing)
  ...
  #       caseimportance: high
  #   
  assign_policy_for_testing.assign_actions_to_event("Host Compliance Check", {"Mark as Non-Compliant" => true})
  host.check_compliance()
  raise unless !host.is_compliant
end
