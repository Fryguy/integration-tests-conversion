...
pytestmark = [pytest.mark.tier(3), test_requirements.general_ui, pytest.mark.usefixtures("setup_provider"), pytest.mark.provider([InfraProvider], required_fields: ["remove_test"], scope: "module")]
...
def test_delete_resource_pool_appear_after_refresh(provider, appliance)
  ...
  test_resourcepool = appliance.collections.resource_pools.instantiate(name: resourcepool_name, provider: provider)
...
def test_delete_datastore_appear_after_refresh(provider, appliance)
  ...
  wait_for(lambda{|| !test_datastore.exists}, delay: 20, timeout: 1200, message: "Wait datastore to disappear", fail_func: test_datastore.browser.refresh)
  provider.refresh_provider_relationships()
  wait_for(lambda{|| test_datastore.exists}, delay: 20, timeout: 1200, message: "Wait datastore to appear", fail_func: test_datastore.browser.refresh)
end
