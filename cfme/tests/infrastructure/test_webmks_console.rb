...
pytestmark = [pytest.mark.usefixtures("setup_provider_modscope"), pytest.mark.provider([VMwareProvider], scope: "module", required_flags: ["webmks_console"])]
...
def test_webmks_vm_console(request, appliance, provider, vm_obj, configure_websocket, configure_console_webmks, take_screenshot)
  ...
  raise "VMConsole object should be created" unless vm_obj.vm_console
  ...
  begin
    ...
    raise "VM Console Toggle Full Screen button does not work" unless vm_console.send_fullscreen()
  rescue Exception
