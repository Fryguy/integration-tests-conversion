...
def test_system_image_type_crud(appliance)
  ...
  sys_image_type = collection.create(name: fauxfactory.gen_alphanumeric(8), provision_type: SystemImageType.VM_OR_INSTANCE)
  update(sys_image_type) {
...
def test_system_image_duplicate_name_error_validation(appliance)
  ...
  sys_image_type = collection.create(name: name, provision_type: SystemImageType.VM_OR_INSTANCE)
  error_message = (appliance.version < "5.10") ? "Name has already been taken" : "Name is not unique within region 0"
  pytest.raises(Exception, match: error_message) {
    collection.create(name: name, provision_type: SystemImageType.VM_OR_INSTANCE)
  }
  sys_image_type.delete(cancel: false)
end
def test_name_required_error_validation_system_image(appliance)
  ...
  pytest.raises(Exception, match: "Name is required") {
    collection.create(name: nil, provision_type: SystemImageType.VM_OR_INSTANCE)
  }
end
def test_system_image_type_selective_delete(appliance)
  ...
  #   
  collection = appliance.collections.system_image_types
  sys_image_type1 = collection.create(name: fauxfactory.gen_alphanumeric(8), provision_type: SystemImageType.VM_OR_INSTANCE)
  sys_image_type2 = collection.create(name: fauxfactory.gen_alphanumeric(8), provision_type: SystemImageType.VM_OR_INSTANCE)
