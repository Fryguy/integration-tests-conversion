...
def test_containers_summary_objects(provider, soft_assert)
  ...
  status_box_values = tested_objects.map{|obj|[obj, (view.status_cards(obj.PLURAL.split_p(" ")[-1])).value]}.to_h
  ...
  for obj in tested_objects
    ...
    for ui_val_field in ui_val_fields
      if ui_val_field.include?(obj.PLURAL)
        ui_val = view.entities.summary("Relationships").get_text_of(ui_val_field).to_i
        soft_assert.(sb_val == ui_val, "{}: Mismatch between status box ({}) value in Containers overviewand provider's relationships table ({}):".format(obj.PLURAL, sb_val, ui_val))
      else
        next
      end
    end
