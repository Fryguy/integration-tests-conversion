...
def test_tenant_quota_enforce_via_service_cloud(request, appliance, context, set_roottenant_quota, set_default, custom_prov_data, extra_msg, template_name, catalog_item)
...
def test_service_cloud_tenant_quota_with_default_entry_point(request, appliance, context, set_roottenant_quota, set_default, custom_prov_data, extra_msg, catalog_item)
...
def instance(appliance, provider, small_template, setup_provider)
  ...
  instance = appliance.collections.cloud_instances.instantiate(random_vm_name("pwr-c"), provider, small_template.name)
