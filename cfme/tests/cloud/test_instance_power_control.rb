...
pytestmark = [pytest.mark.tier(2), pytest.mark.long_running, test_requirements.power, pytest.mark.provider([CloudProvider], scope: "function", required_fields: PROVIDER_FIELDS), pytest.mark.usefixtures("setup_provider")]
def create_instance(appliance, provider, template_name)
  instance = appliance.collections.cloud_instances.instantiate(random_vm_name("pwr-c"), provider, template_name)
...
def wait_for_ui_state_refresh(instance, provider, state_change_time, timeout: 900)
  ...
  _wait_for_state_refresh = lambda do
    begin
  ...
  begin
    wait_for(method(:_wait_for_state_refresh), fail_func: _fail_func, num_sec: timeout, delay: 30, message: "Waiting for instance state refresh")
  rescue TimedOutError
    return false
  end
end
def wait_for_power_state_refresh(instance, state_change_time, timeout: 720)
  return wait_for(lambda{|| instance.rest_api_entity.state_changed_on != state_change_time}, num_sec: timeout.to_i, delay: 30, message: "Waiting for instance state refresh").out
end
...
def wait_for_instance_state(soft_assert, instance, state)
  ...
  if ["started", "running"].include?(state)
    ...
    if state == "stopped"
      ...
      if is_bool(state == "suspended" && instance.mgmt.system.can_suspend)
        desired_mgmt_state = VmState.SUSPENDED
        desired_ui_state = instance.STATE_SUSPENDED
      else
        if is_bool(state == "paused" && instance.mgmt.system.can_pause)
          desired_mgmt_state = VmState.PAUSED
          desired_ui_state = instance.STATE_PAUSED
        else
          if state == "terminated"
            desired_mgmt_state = nil
            desired_ui_state = [instance.STATE_TERMINATED, instance.STATE_ARCHIVED, instance.STATE_UNKNOWN]
          else
            raise TypeError, "Invalid instance state type of '{}' for provider '{}'".format(state, instance.provider)
          end
        end
      end
    end
...
def test_quadicon_terminate_cancel(provider, testing_instance, ensure_vm_running, soft_assert)
  ...
  #       tags: power
  #   
  testing_instance.power_control_from_cfme(option: testing_instance.TERMINATE, cancel: true, from_details: false)
...
def test_quadicon_terminate(appliance, provider, testing_instance, ensure_vm_running, soft_assert)
  ...
  soft_assert.(testing_instance.wait_for_instance_state_change(desired_state: [testing_instance.STATE_TERMINATED, testing_instance.STATE_ARCHIVED, testing_instance.STATE_UNKNOWN], timeout: 1200))
end
...
def test_start(appliance, provider, testing_instance, ensure_vm_stopped, soft_assert)
  ...
  #       tags: power
  #   
  testing_instance.wait_for_instance_state_change(desired_state: testing_instance.STATE_OFF, timeout: 900)
...
def test_power_on_or_off_multiple(provider, testing_instance, testing_instance2, soft_assert)
  ...
  _get_view_with_icons_checked = lambda do
  ...
  wait_for_instance_state(soft_assert, testing_instance, state: "stopped")
  wait_for_instance_state(soft_assert, method(:testing_instance2), state: "stopped")
  view = _get_view_with_icons_checked.call()
  ...
  wait_for_instance_state(soft_assert, testing_instance, state: "started")
  wait_for_instance_state(soft_assert, method(:testing_instance2), state: "started")
end
...
def test_hard_reboot_unsupported(appliance, testing_instance)
  ...
  #       tags: power
  #   
  testing_instance.power_control_from_cfme(option: testing_instance.HARD_REBOOT, from_details: false)
  message = (appliance.version < "5.10") ? "Reset does not apply to at least one of the selected items" : "Reset action does not apply to selected items"
...
def test_unpause(appliance, provider, testing_instance, ensure_vm_paused, soft_assert)
  ...
  appliance.browser.create_view(BaseLoggedInPage).flash.assert_success_message(text: "Start initiated", partial: true)
  wait_for_instance_state(soft_assert, testing_instance, state: "started")
end
...
def test_instance_power_options_from_off(provider, testing_instance, ensure_vm_stopped, soft_assert)
  ...
  #       tags: power
  #   
  testing_instance.wait_for_instance_state_change(desired_state: testing_instance.STATE_OFF, timeout: 1200)
  check_power_options(soft_assert, testing_instance, "off")
end
class TestInstanceRESTAPI
  #  Tests using the /api/instances collection. 
  def test_stop(provider, testing_instance, ensure_vm_running, soft_assert, appliance, from_detail)
    #  Tests instance stop
    # 
    #     Metadata:
    ...
    raise unless testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_OFF)
    wait_for_instance_state(soft_assert, testing_instance, state: "stopped")
  end
  def test_start(provider, testing_instance, ensure_vm_stopped, soft_assert, appliance, from_detail)
    #  Tests instance start
    # 
    #     Metadata:
    #         test_flag: power_control, provision, rest
    ...
    #         initialEstimate: 1/4h
    #     
    testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_OFF, timeout: 1200)
    vm = testing_instance.rest_api_entity
    if is_bool(from_detail)
      vm.action.start()
    else
    ...
    assert_response(appliance.rest_api)
    raise unless testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_ON)
    wait_for_instance_state(soft_assert, testing_instance, state: "started")
  end
  def test_soft_reboot(provider, testing_instance, soft_assert, ensure_vm_running, appliance, from_detail)
    #  Tests instance soft reboot
    # 
    #     Metadata:
    #         test_flag: power_control, provision, rest
  ...
  def test_hard_reboot(provider, testing_instance, soft_assert, ensure_vm_running, appliance, from_detail)
    #  Tests instance hard reboot
    # 
    #     Metadata:
    #         test_flag: power_control, provision, rest
    ...
    assert_response(appliance.rest_api)
    raise unless testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_ON, timeout: 720)
    wait_for_instance_state(soft_assert, testing_instance, state: "started")
  end
  def test_suspend_resume(provider, testing_instance, soft_assert, ensure_vm_running, appliance, from_detail)
    ...
    assert_response(appliance.rest_api)
    raise unless testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_SUSPENDED, delay: 15)
    wait_for_instance_state(soft_assert, testing_instance, state: "suspended")
    if is_bool(from_detail)
      vm.action.start()
    else
    ...
    assert_response(appliance.rest_api)
    raise unless testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_ON, delay: 15)
    wait_for_instance_state(soft_assert, testing_instance, state: "started")
  end
  def test_pause_unpause(provider, testing_instance, soft_assert, ensure_vm_running, appliance, from_detail)
    ...
    assert_response(appliance.rest_api)
    raise unless testing_instance.wait_for_power_state_change_rest(desired_state: testing_instance.STATE_PAUSED, delay: 15)
  ...
  def test_terminate(provider, testing_instance, soft_assert, ensure_vm_running, appliance, from_detail)
    ...
    assert_response(appliance.rest_api)
    wait_for_instance_state(soft_assert, testing_instance, state: "terminated")
    terminated_states = [testing_instance.STATE_TERMINATED, testing_instance.STATE_ARCHIVED, testing_instance.STATE_UNKNOWN]
    vm.reload()
    soft_assert.(terminated_states.include?(vm.power_state), "instance not terminated")
  end
end
def test_power_options_on_archived_instance_all_page(testing_instance)
  ...
  for action in view.toolbar.power.to_a
    if is_bool(action == "Resume" && BZ(1738584, forced_streams: ["5.10", "5.11"]).blocks)
      next
    end
