...
def management_event_method(management_event_class, ansible_repository)
  return management_event_class.methods.create(name: fauxfactory.gen_alphanumeric(start: "meth_"), location: "playbook", repository: ansible_repository.name, playbook: "copy_file_example.yml", machine_credential: "CFME Default Credential", playbook_input_parameters: [["key", "value", "string"]])
end
def management_event_instance(management_event_class, management_event_method)
  return management_event_class.instances.create(name: fauxfactory.gen_alphanumeric(start: "inst_"), description: fauxfactory.gen_alphanumeric(), fields: {"meth1" => {"value" => management_event_method.name}})
end
...
def test_automate_ansible_playbook_method_type(request, appliance, ansible_repository, domain, namespace, klass)
...
def test_embedded_ansible_custom_button_localhost(create_vm_modscope, custom_vm_button, appliance, ansible_service_request_funcscope, ansible_service_funcscope, ansible_catalog_item)
  # 
  #   Polarion:
  #       assignee: gtalreja
  #       casecomponent: Ansible
  ...
  raise unless hosts == "localhost"
  status = (appliance.version < "5.11") ? "successful" : "Finished"
  raise unless view.provisioning.results.get_text_of("Status") == status
end
def test_embedded_ansible_custom_button_target_machine(create_vm_modscope, custom_vm_button, ansible_credential, appliance, ansible_service_request_funcscope, ansible_service_funcscope)
  # 
  #   Polarion:
  #       assignee: gtalreja
  #       casecomponent: Ansible
  ...
  view = navigate_to(ansible_service_funcscope, "Details")
  hosts = view.provisioning.details.get_text_of("Hosts")
  raise unless hosts == create_vm_modscope.ip_address
  status = (appliance.version < "5.11") ? "successful" : "Finished"
  raise unless view.provisioning.results.get_text_of("Status") == status
end
def test_embedded_ansible_custom_button_specific_hosts(create_vm_modscope, custom_vm_button, ansible_credential, appliance, ansible_service_request_funcscope, ansible_service_funcscope)
  # 
  #   Polarion:
  #       assignee: gtalreja
  #       casecomponent: Ansible
...
def test_alert_run_ansible_playbook(create_vm_modscope, alert_profile, request, appliance)
  ...
  request.addfinalizer(lambda{|| appliance.ssh_client.run_command("[[ -f \"/var/tmp/modified-release\" ]] && rm -f \"/var/tmp/modified-release\"")})
  begin
    wait_for(lambda{|| (appliance.ssh_client.run_command("[ -f \"/var/tmp/modified-release\" ]")).success}, timeout: 60)
...
def test_import_domain_containing_playbook_method(request, appliance, setup_ansible_repository, import_data)
