...
def test_redfish_power_buttons(physical_server, provider)
  ...
  #       initialEstimate: 1/4h
  #   
  power_actions = [["power_off", lambda{|| physical_server.power_off()}], ["power_on", lambda{|| physical_server.power_on()}], ["power_off_now", lambda{|| physical_server.power_off_immediately()}], ["restart", lambda{|| physical_server.restart()}], ["restart_now", lambda{|| physical_server.restart_immediately()}], ["turn_on_loc_led", lambda{|| physical_server.turn_on_led()}], ["turn_off_loc_led", lambda{|| physical_server.turn_off_led()}], ["blink_loc_led", lambda{|| physical_server.turn_blink_led()}]]
  for (action_name, action) in power_actions
    action()
    assert_message(physical_server, "Requested {} of selected item.".format(action_name))
  end
end
