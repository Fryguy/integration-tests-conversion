...
ACCESS_RULES_VMS = VersionPicker({"LOWEST" => "Access Rules for all Virtual Machines", "5.11" => "All VM and Instance Access Rules"})
...
def new_user(appliance, groups, name: nil, credential: nil)
  name = name || fauxfactory.gen_alphanumeric(start: "user_")
  credential = credential || new_credential()
  user = appliance.collections.users.create(name: name, credential: credential, email: "xyz@redhat.com", groups: groups, cost_center: "Workload", value_assign: "Database")
  return user
end
def new_role(appliance, name: nil)
  name = name || fauxfactory.gen_alphanumeric(start: "role_")
  return appliance.collections.roles.create(name: name, vm_restriction: "None")
end
...
def check_item_visibility(tag)
  _check_item_visibility = lambda do |item, user_restricted|
    item.add_tag(tag)
    user_restricted {
      raise unless item.exists
    }
    item.remove_tag(tag)
    user_restricted {
      raise unless !item.exists
...
def tag_value(appliance, category, tag, request)
  ...
  if tag_type == "tag"
    ...
    tag_for_create = "fill_tag(My Company Tags : {}, {})".format(category.display_name, tag.display_name)
...
def test_user_assign_multiple_groups(appliance, request)
  ...
  #       tags: rbac
  #   
  group_names = ["EvmGroup-user", "EvmGroup-administrator", "EvmGroup-user_self_service", "EvmGroup-desktop"]
  group_collection = appliance.collections.groups
  group_list = group_names.map{|name| group_collection.instantiate(description: name)}
  ...
  user {
    ...
    raise "User {} assigned groups {} are different from expected groups {}".format(user, view.group_names, group_names) unless Set.new(assigned_groups) == Set.new(group_names)
  }
end
def test_user_change_groups(appliance)
  ...
  #       casecomponent: Configuration
  #   
  group_names = ["EvmGroup-super_administrator", "EvmGroup-administrator", "EvmGroup-approver", "EvmGroup-auditor", "EvmGroup-desktop", "EvmGroup-operator", "EvmGroup-security", "EvmGroup-user", "EvmGroup-vm_user"]
  group_collection = appliance.collections.groups
  group_list = group_names.map{|name| group_collection.instantiate(description: name)}
  ...
  test_user {
    ...
    group_test_list = group_names.select{|name| name != orig_group}.map{|name| name} + [orig_group]
    for group in group_test_list
      view.change_group(group)
      raise "User failed to change current group from {} to {}".format(view.current_groupname, group) unless group == view.current_groupname
    end
  }
end
...
def test_username_required_error_validation(appliance)
  ...
  pytest.raises(Exception, match: "Name can't be blank") {
    appliance.collections.users.create(name: "", credential: new_credential(), email: "xyz@redhat.com", groups: [group])
  }
end
def test_userid_required_error_validation(appliance)
  ...
  pytest.raises(Exception, match: "Userid can't be blank") {
    appliance.collections.users.create(name: fauxfactory.gen_alphanumeric(start: "user_"), credential: Credential(principal: "", secret: "redhat"), email: "xyz@redhat.com", groups: [group])
  }
...
def test_user_password_required_error_validation(appliance)
  ...
  pytest.raises(Exception, match: check) {
    appliance.collections.users.create(name: fauxfactory.gen_alphanumeric(start: "user_"), credential: Credential(principal: fauxfactory.gen_alphanumeric(start: "uid"), secret: nil), email: "xyz@redhat.com", groups: [group])
  }
  navigate_to(appliance.server, "Dashboard")
end
def test_user_group_error_validation(appliance)
  ...
  #       tags: rbac
  #   
  pytest.raises(Exception, match: "A User must be assigned to a Group") {
    appliance.collections.users.create(name: fauxfactory.gen_alphanumeric(start: "user_"), credential: new_credential(), email: "xyz@redhat.com", groups: [""])
  }
end
def test_user_email_error_validation(appliance)
  ...
  pytest.raises(Exception, match: "Email must be a valid email address") {
    appliance.collections.users.create(name: fauxfactory.gen_alphanumeric(start: "user_"), credential: new_credential(), email: "xyzdhat.com", groups: group)
  }
end
def test_user_edit_tag(appliance, tag)
  ...
  user = new_user(appliance, [group])
  user.add_tag(tag)
  raise "Assigned tag was not found on the details page" unless user.get_tags().map{|tag_available| tag_available.category.display_name == tag.category.display_name && tag_available.display_name == tag.display_name}.is_any?
  user.delete()
end
def test_user_remove_tag(appliance)
  ...
  raise "Remove User tag failed" unless !user.get_tags().map{|tag| tag.category.display_name == added_tag.category.display_name && tag.display_name == added_tag.display_name}.is_any?
  user.delete()
end
...
def test_group_crud_with_tag(appliance, provider, setup_provider, tag_value)
  ...
  group_collection = appliance.collections.groups
  group = group_collection.create(description: fauxfactory.gen_alphanumeric(start: "grp_"), role: "EvmRole-approver", tag: tag_for_create, host_cluster: [[provider.data["name"]], true], vm_template: [[provider.data["name"], provider.data["datacenters"][0], path], true])
  update(group) {
    group.tag = tag_for_update
    group.host_cluster = [[provider.data["name"]], false]
    group.vm_template = [[provider.data["name"], provider.data["datacenters"][0], path], false]
  }
  group.delete()
end
...
def test_group_edit_tag(appliance)
  ...
  raise "Group edit tag failed" unless group.get_tags().map{|tag| tag.category.display_name == added_tag.category.display_name && tag.display_name == added_tag.display_name}.is_any?
  group.delete()
end
def test_group_remove_tag(appliance)
  ...
  raise "Remove Group User tag failed" unless !group.get_tags().map{|tag| tag.category.display_name == added_tag.category.display_name && tag.display_name == added_tag.display_name}.is_any?
  group.delete()
end
def test_group_description_required_error_validation(appliance)
  ...
  group_collection = appliance.collections.groups
  pytest.raises(Exception, match: error_text) {
    group_collection.create(description: nil, role: "EvmRole-approver")
  }
...
def test_rolename_required_error_validation(appliance)
  ...
  raise unless view.add_button.disabled
...
def test_permission_edit(appliance, request, product_features)
  ...
  role = appliance.collections.roles.create(name: role_name, vm_restriction: nil, product_features: [[["Everything"], false]] + product_features.map{|k| [k, true]})
  group_description = fauxfactory.gen_alphanumeric(start: "grp_")
  group_collection = appliance.collections.groups
  group = group_collection.create(description: group_description, role: role.name)
  user = new_user(appliance, [group])
  ...
  role.update({"product_features" => [[["Everything"], true]] + product_features.map{|k| [k, false]}})
  user {
    pytest.raises(Exception, match: "Could not find an element") {
      navigate_to(appliance.collections.infra_vms, "VMsOnly")
    }
  }
  _delete_user_group_role = lambda do
    for item in [user, group, role]
      item.delete()
    end
  end
end
def _mk_role(appliance, name: nil, vm_restriction: nil, product_features: nil)
  # Create a thunk that returns a Role object to be used for perm
  #      testing.  name=None will generate a random name
  # 
  #   
  name = name || fauxfactory.gen_alphanumeric(start: "role_")
  return appliance.collections.roles.create(name: name, vm_restriction: vm_restriction, product_features: product_features)
end
def _go_to(cls_or_obj, dest: "All")
  # Create a thunk that navigates to the given destination
  nav = lambda do |appliance|
...
def test_permissions(appliance, product_features, allowed_actions, disallowed_actions)
  ...
  begin
    user {
      navigate_to(appliance.server, "LoggedIn")
      for (name, action_thunk) in sorted(allowed_actions.to_a())
        begin
...
def single_task_permission_test(appliance, product_features, actions)
  ...
  test_prod_features = [[["Everything"], false]] + product_features.map{|f| [f, true]}
  test_permissions(appliance, test_prod_features, actions, {})
  test_prod_features = [[["Everything"], true]] + product_features.map{|f| [f, false]}
  test_permissions(appliance, test_prod_features, {}, actions)
end
def test_permissions_role_crud(appliance)
  ...
  #       tags: rbac
  #   
  configuration = (appliance.version < "5.11") ? ["Everything", "Settings", "Configuration"] : ["Everything", "Main Configuration"]
  single_task_permission_test(appliance, [configuration, ["Everything", "Services", "Catalogs Explorer"]], {"Role CRUD" => test_role_crud})
end
def test_permissions_vm_provisioning(appliance, provider, setup_provider)
  ...
  #       tags: rbac
  #   
  features = [["Everything", "Compute", "Infrastructure", "Virtual Machines", "Accordions"], ["Everything", ACCESS_RULES_VMS, "VM Access Rules", "Modify", "Provision VMs"]]
  single_task_permission_test(appliance, features, {"Provision VM" => _test_vm_provision})
end
def test_user_change_password(appliance, request)
  ...
  user {
    ...
    navigate_to(appliance.server, "LoggedIn")
    raise unless appliance.server.current_full_name() == user.name
  }
  appliance.server.login_admin()
  update(user) {
    user.credential = Credential(principal: user.credential.principal, secret: "another_very_secret", verify_secret: "another_very_secret")
  }
  user {
    appliance.server.logout()
    navigate_to(appliance.server, "LoggedIn")
...
def test_copied_user_password_inheritance(appliance, request)
  ...
  raise unless view.password_txt.value == "" && view.password_verify_txt.value == ""
  view.cancel_button.click()
end
def test_delete_default_tenant(appliance)
  ...
  raise unless roottenant.exists
end
def test_superadmin_tenant_crud(request, appliance)
  ...
  tenant = tenant_collection.create(name: fauxfactory.gen_alphanumeric(start: "tenant1"), description: "tenant1 description", parent: tenant_collection.get_root_tenant())
  _delete_tenant = lambda do
...
def test_superadmin_tenant_project_crud(request, appliance)
  ...
  tenant = tenant_collection.create(name: fauxfactory.gen_alphanumeric(start: "tenant1"), description: "tenant1 description", parent: tenant_collection.get_root_tenant())
  project = project_collection.create(name: fauxfactory.gen_alphanumeric(12, start: "project1"), description: "project1 description", parent: project_collection.get_root_tenant())
  _delete_tenant_and_project = lambda do
    for item in [project, tenant]
      if is_bool(item.exists)
        item.delete()
      end
    end
  end
...
def test_unique_name_on_parent_level(request, appliance, collection_name)
  ...
  #   
  object_collection = appliance.collections.getattr(collection_name)
  name_of_tenant = fauxfactory.gen_alphanumeric(15, start: "tenant_")
  tenant_description = "description"
  tenant = object_collection.create(name: name_of_tenant, description: tenant_description, parent: object_collection.get_root_tenant())
  pytest.raises(Exception, match: "Failed to add a new tenant resource - Tenant: Name should be unique per parent") {
    tenant2 = object_collection.create(name: name_of_tenant, description: tenant_description, parent: object_collection.get_root_tenant())
  }
  tenant.delete()
...
def test_tenantadmin_group_crud(child_tenant_admin_user, tenant_role, child_tenant, request, appliance)
  # 
...
def test_tenant_unique_catalog(appliance, request, catalog_obj)
  ...
  view.fill({"name" => catalog_obj.name, "description" => catalog_obj.description})
  if appliance.version > "5.11"
    raise unless view.name.help_block == msg
  else
    view.add_button.click()
...
def test_tenantadmin_user_crud(child_tenant_admin_user, tenant_role, child_tenant, request, appliance)
  # 
...
def test_tenant_unique_automation_domain_name_on_parent_level(appliance, request)
  ...
  domain1.delete()
  _delete_domain = lambda do
    if is_bool(domain1.exists)
      domain1.delete()
    end
    begin
      if is_bool(domain2.exists)
        domain2.delete()
      end
    rescue NameError
      # pass
...
def test_tenant_ldap_group_switch_between_tenants(appliance, setup_openldap_auth_provider, setup_openldap_user_group, soft_assert)
  # 
...
def test_create_group_console_no(appliance, request)
  ...
  # 
  #   
  cg = appliance.collections.categories.create(name: fauxfactory.gen_alphanumeric(10, start: "name_").downcase(), description: fauxfactory.gen_alphanumeric(32, start: "long_desc_"), display_name: fauxfactory.gen_alphanumeric(32, start: "desc_"), show_in_console: false)
  request.addfinalizer(cg.delete_if_exists)
  msg = ".*FATAL -- : Error caught: [NoMethodError] undefined method.*"
  (LogValidator("/var/www/miq/vmdb/log/production.log", failure_patterns: [msg])).waiting(timeout: 120) {
    group = appliance.collections.groups.create(description: fauxfactory.gen_alphanumeric(22, "group_description_"), role: "EvmRole-vm_user")
    request.addfinalizer(group.delete_if_exists)
  }
end
def test_select_edit_group(appliance, request)
  ...
  (LogValidator("/var/www/miq/vmdb/log/production.log", failure_patterns: [msg])).waiting(timeout: 120) {
    ...
    raise "\"Edit the selected Group\" button is disabled" unless view.toolbar.configuration.item_enabled("Edit the selected Group")
  }
end
