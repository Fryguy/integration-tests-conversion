...
SCHEDULES_REPORT_DIR = "schedules_crud".data_path.join
TIMER = {"monthly" => {"run" => "Monthly", "timer_month" => "2 Months", "hour" => "12", "minute" => "5", "time_zone" => "(GMT+10:00) Melbourne"}, "hourly" => {"run" => "Hourly", "timer_hour" => "6 Hours", "hour" => "12", "minute" => "5", "time_zone" => "(GMT+10:00) Melbourne"}, "daily" => {"run" => "Daily", "timer_day" => "2 Days", "hour" => "12", "minute" => "5", "time_zone" => "(GMT+10:00) Melbourne"}, "weekly" => {"run" => "Weekly", "timer_week" => "3 Weeks", "hour" => "12", "minute" => "5", "time_zone" => "(GMT+10:00) Melbourne"}, "once" => {"run" => "Once", "hour" => "12", "minute" => "5", "time_zone" => "(GMT+10:00) Melbourne"}}
INVALID_EMAILS = {"string" => fauxfactory.gen_alpha(), "multiple-dots" => "{name}..{name}@example..com".format(name: fauxfactory.gen_alpha(5)), "brackets" => "{name}@example.com({name})".format(name: fauxfactory.gen_alpha(5)), "leading-dot" => ".{name}@example.com".format(name: fauxfactory.gen_alpha(5)), "dash" => ("{name}@-example.com").format(name: fauxfactory.gen_alpha(5)), "missing-@" => "{name}.example.com".format(name: fauxfactory.gen_alpha(5)), "trailing-dot" => "{name}.@example.com".format(name: fauxfactory.gen_alpha(5)), "missing-username" => "@example.com"}
def schedule_files()
  result = []
  if is_bool(!SCHEDULES_REPORT_DIR.exists)
...
def schedule_data(request)
  request.param.SCHEDULES_REPORT_DIR.join.open(mode: "r") {|rep_yaml|
...
def user(appliance, request)
  users,user_data = _users(request, appliance, name: "Sherlock Holmes", email: "shholmes@redhat.com", userid: "shholmes", password: "smartvm", group: "EvmGroup-super_administrator")
  return users[0]
end
def test_schedule_queue(appliance, request, interval, schedule_data)
  ...
  schedule.queue()
  raise unless schedule.timer == TIMER[interval]
...
def test_reports_create_schedule_send_report(smtp_test, schedule)
  ...
  #   
  schedule.queue()
  emails_sent = schedule.email.get("to_emails", []).join(",")
  initial_count = smtp_test.get_emails().size
  wait_for(lambda{|| smtp_test.get_emails().size > initial_count}, num_sec: 90, delay: 5)
  raise unless smtp_test.get_emails(to_address: emails_sent).size == 1
end
...
def test_miq_schedule_validation_failed(temp_appliance_preconfig)
  ...
  (LogValidator("/var/www/miq/vmdb/log/evm.log", failure_patterns: [".* ERROR .*Validation failed: MiqSchedule:.*Name has already been taken.*Method.*"], matched_patterns: [".*INFO.* Widget: .*chart_server_availability.* file has been .* disk.*", ".*INFO.* : MIQ.*EvmDatabase.seed.* Seeding MiqAction.*"])).waiting() {
    appliance.db.restore_database(db_path: dump_destination, is_major: bool(appliance.version > "5.11"))
  }
end
