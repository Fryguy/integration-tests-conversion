...
function test_schedule_migration(appliance, provider, mapping_data_vm_obj_mini, soft_assert, request) {
  // 
  ...
  let view = navigate_to(migration_plan_collection, "NotStarted");
  view.plans_not_started_list.schedule_migration(migration_plan.name);
  soft_assert.call(view.plans_not_started_list.get_clock(migration_plan.name).include("Migration scheduled"));
