...
let pytestmark = [test_requirements.cockpit];

function test_cockpit_server_role(appliance, provider, setup_provider, create_vm, enable) {
  ...
  if (is_bool(enable)) {
    ...
    wait_for(
      ...
      {delay: 20, timeout: 300}
    );

    let view = navigate_to(create_vm, "Details");
    if (!view.toolbar.access.item_enabled("Web Console")) throw new ();
    ...
    let view = navigate_to(create_vm, "Details");
    let access = view.toolbar.access;
    if (!!access.is_enabled && !!access.item_enabled("Web Console")) throw new ()
  }
}
