...
function new_vm(appliance, provider, setup_provider, small_template_modscope) {
...
function test_add_vm_to_service(service_vm, request, copy_domain, new_vm, appliance) {
  ...
  let method = copy_domain.namespaces.instantiate({name: "System"}).classes.instantiate({name: "Request"}).methods.create({
    name: "InspectMe",
    location: "inline",
    script: method_torso
  });
