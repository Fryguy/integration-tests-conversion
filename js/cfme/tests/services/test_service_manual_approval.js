...
function service_retirement_request(domain) {
  ...
  let method = domain.namespaces.instantiate({name: "Service"}).namespaces.instantiate({name: "Retirement"}).namespaces.instantiate({name: "StateMachines"}).classes.instantiate({name: "ServiceRetirementRequestApproval"}).instances.instantiate({name: "Default"});
...
function create_domain(request, appliance) {
  // Create new domain and copy instance from ManageIQ to this domain
  let dc = DomainCollection(appliance);

  let new_domain = dc.create({
    name: fauxfactory.gen_alphanumeric(12, {start: "domain_"}),
    enabled: true
  });

  request.addfinalizer(new_domain.delete_if_exists);
  let instance = dc.instantiate({name: "ManageIQ"}).namespaces.instantiate({name: "Service"}).namespaces.instantiate({name: "Provisioning"}).namespaces.instantiate({name: "StateMachines"}).classes.instantiate({name: "ServiceProvisionRequestApproval"}).instances.instantiate({name: "Default"});
  instance.copy_to(new_domain);
  return new_domain
};

function modify_instance(create_domain) {
  // Modify the instance in new domain to change it to manual approval instead of auto
  let instance = create_domain.namespaces.instantiate({name: "Service"}).namespaces.instantiate({name: "Provisioning"}).namespaces.instantiate({name: "StateMachines"}).classes.instantiate({name: "ServiceProvisionRequestApproval"}).instances.instantiate({name: "Default"});
...
function test_service_manual_approval(appliance, provider, modify_instance, catalog_item, request) {
  ...
  if (service_request.row.approval_state.text != "Pending Approval") {
    throw new ()
  }
};

function test_service_retire_manual_approval(request, appliance, service_retirement_request, service_vm) {
  ...
  appliance.context.use(ViaUI, () => {
    ...
    let request_description = `Service Retire for: ${service.name}`;
    ...
    let view = appliance.browser.create_view(RequestDetailsToolBar);

    if (is_bool(!view.approve.is_displayed && BZ(
      1721479,
    ...
    if (service_request.row.approval_state.text != "Approved") throw new ();
    service_request.wait_for_request();
    let msg = `Request failed with the message ${service_request.rest.message}`;
    request.addfinalizer(service_request.remove_request);
    if (!service_request.is_succeeded()) throw msg
  })
}
