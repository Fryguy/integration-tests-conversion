...
function test_creating_entities_emits_events(register_event, with_nuage_sandbox) {
  // 
  ...
  //   
  let listener = register_event;
  let sandbox = with_nuage_sandbox;

  expect_event(
    listener,
    "nuage_enterprise_create",
    sandbox.enterprise.id
  );

  expect_event(
    listener,
    "nuage_domaintemplate_create",
    sandbox.template.id
  );
  ...
  expect_event(
    listener,
    "nuage_l2domaintemplate_create",
    sandbox.l2_template.id
  );
  ...
  expect_event(
    listener,
    "nuage_vport_create",
    sandbox.l2_vm_vport.id,
    {comment: "L2 vm"}
  );

  expect_event(
    listener,
    "nuage_policygroup_create",
    sandbox.group.id,
    {comment: "L1"}
  );
...
function test_creating_entities_triggers_targeted_refresh(targeted_refresh, with_nuage_sandbox) {
  // 
  ...
  targeted_refresh.timeout(() => {
    ...
    targeted_refresh.register_target(
      sandbox.cont_vport.id,
      "Container vPort (L3)"
    );
...
function expect_event(listener, event_type, entity_id, { comment = "" }) {
  // 
  ...
  //   Returns: None
  //   
  let cmp_function = (_, full_data) => full_data.entities[0].ID == entity_id;

  listener(
    {
      full_data: `${event_type} [ID=${entity_id}] ${comment}`,
      cmp_func: cmp_function
    },

    {source: "NUAGE", event_type, first_event: true}
  )
}
