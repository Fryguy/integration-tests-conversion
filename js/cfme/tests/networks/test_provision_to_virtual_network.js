...
function network(provider, appliance) {
  ...
  let test_name = fauxfactory.gen_alphanumeric(
    18,
    {start: "test_network_"}
  );

  let net_manager = `${provider.name} Network Manager`;
  let collection = appliance.collections.network_providers;

  let network_provider = collection.instantiate({
  ...
  let ovn_network = collection.create(
    test_name,
    "tenant",
    network_provider,
    net_manager,
  ...
  yield(ovn_network);
  if (is_bool(ovn_network.exists)) ovn_network.delete()
};

function test_provision_vm_to_virtual_network(appliance, setup_provider, provider, request, provisioning, network) {
  ...
  let provisioning_data = {
    ...
    environment: {vm_name: vm_name, automatic_placement: true},
    network: {vlan: partial_match(network.name)}
  };
  ...
  wait_for(
...

    {
      ...
      message: `Cannot do provision for vm ${vm_name}.`
    }
  )
}
