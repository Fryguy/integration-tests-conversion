...
let pytestmark = [
  ...
  pytest.mark.provider(
    ...
    {required_fields: ["templates"], scope: "module"}
  )
];

function prepare_new_config(orig_config, change_type) {
  ...
  } else if (change_type == "sockets") {
    new_config.hw.sockets = new_config.hw.sockets + 1
  } else if (change_type == "memory") {
...
function reconfigure_vm(vm, config) {
  // Reconfigure VM to have the supplies config.
  let reconfigure_request = vm.reconfigure(config);

  wait_for(reconfigure_request.is_succeeded, {
    timeout: 360,
    delay: 45,
    message: "confirm that vm was reconfigured"
  });

  wait_for(() => vm.configuration == config, {
    timeout: 360,
    delay: 45,
    fail_func: vm.refresh_relationships,

    message: "confirm that config was applied. Hardware: {}, disks: {}".format(
      vars(config.hw),
      config.disks
    )
  })
...
function full_vm(appliance, provider, full_template) {
...
function ensure_vm_stopped(full_vm) {
  if (is_bool(full_vm.is_pwr_option_available_in_cfme(full_vm.POWER_OFF))) {
    ...
    throw new Exception("Unknown power state - unable to continue!")
  }
};

function ensure_vm_running(full_vm) {
...
function vm_state(request, full_vm) {
  ...
  } else if (is_bool(full_vm.is_pwr_option_available_in_cfme(full_vm.POWER_ON))) {
    full_vm.mgmt.ensure_state(VmState.RUNNING);
    full_vm.wait_for_vm_state_change(full_vm.STATE_ON)
  } else {
    throw new Exception("Unknown power state - unable to continue!")
...
function enable_hot_plugin(provider, full_vm, ensure_vm_stopped) {
  if (is_bool(provider.one_of(VMwareProvider))) {
    let vm = provider.mgmt.get_vm(full_vm.name);
    vm.cpu_hot_plug = true;
    vm.memory_hot_plug = true
  }
};

function test_vm_reconfig_add_remove_hw_cold(provider, full_vm, ensure_vm_stopped, change_type) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Infra
  ...
  //       tags: reconfigure
  //   
  let orig_config = full_vm.configuration.copy();
  let new_config = prepare_new_config(orig_config, change_type);
  reconfigure_vm(full_vm, new_config);
  reconfigure_vm(full_vm, orig_config)
};

function test_vm_reconfig_add_remove_disk(provider, full_vm, vm_state, disk_type, disk_mode) {
  // 
  //   Polarion:
  //       assignee: nansari
  ...
  let add_disk_request = full_vm.reconfigure(new_config);
  ...
  wait_for(
    () => full_vm.configuration.num_disks == new_config.num_disks,
  ...
  let msg = "Disk wasn't added to VM config";
  if (full_vm.configuration.num_disks != new_config.num_disks) throw msg;
  let remove_disk_request = full_vm.reconfigure(orig_config);
  ...
  wait_for(
    () => full_vm.configuration.num_disks == orig_config.num_disks,
  ...
  if (full_vm.configuration.num_disks != orig_config.num_disks) throw msg
};

function test_reconfig_vm_negative_cancel(provider, full_vm, ensure_vm_stopped) {
  ...
  //       tags: reconfigure
  //   
  let config_vm = full_vm.configuration.copy();
...
function test_vm_reconfig_add_remove_hw_hot(provider, full_vm, enable_hot_plugin, ensure_vm_running, change_type) {
  ...
  let orig_config = full_vm.configuration.copy();
  let new_config = prepare_new_config(orig_config, change_type);
  if (vars(orig_config.hw) == vars(new_config.hw)) throw new ();
  reconfigure_vm(full_vm, new_config);
  if (vars(full_vm.configuration.hw) != vars(new_config.hw)) throw new ();
...
function test_vm_reconfig_resize_disk(appliance, full_vm, vm_state, disk_type, disk_mode) {
  ...
  //        casecomponent: Infra
  //    
  let initial_disks = full_vm.configuration.disks.map(disk => disk.filename);
  ...
  let add_data = [{
    ...
    dependent: !disk_mode.include("independent"),
    bootable: false
  }];

  vm_reconfig_via_rest(
    appliance,
    "disk_add",
    full_vm.rest_api_entity.id,
    add_data
  );

  if (!wait_for(
    () => full_vm.configuration.num_disks > initial_disks.size,
    {fail_func: full_vm.refresh_relationships, delay: 5, timeout: 200}
  )) throw new ();

  let disks_present = full_vm.configuration.disks.map(disk => disk.filename);

  try {
    let disk_added = new Set(disks_present) - new Set(initial_disks).to_a[0]
  } catch ($EXCEPTION) {
  ...
  let disk_size = 500;
  let new_config = full_vm.configuration.copy();

  new_config.resize_disk({
    size_unit: "MB",
    size: disk_size,
    filename: disk_added
  });

  let resize_disk_request = full_vm.reconfigure({new_configuration: new_config});

  wait_for(
    resize_disk_request.is_succeeded,
    {timeout: 360, delay: 45, message: "confirm that disk was Resize"}
  );

  let view = navigate_to(full_vm, "Reconfigure");

  if (view.disks_table.row({name: disk_added}).Size.text.to_i != disk_size) {
    throw new ()
  }
};

function test_vm_reconfig_resize_disk_snapshot(request, disk_type, disk_mode, full_vm, { memory = false }) {
  // 
  // 
  //   Bugzilla:
  //       1631448
  ...
  let view = navigate_to(full_vm, "Reconfigure");
  let row = view.disks_table.rows().map(r => r) // next;
  if (!row.actions.widget.is_enabled) throw new ();
  if (!!row[9].widget.is_displayed) throw new ()
};

function test_vm_reconfig_add_remove_network_adapters(request, adapters_type, full_vm) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       initialEstimate: 1/6h
  ...
  new_config.add_network_adapter(
    `Network adapter ${orig_config.num_network_adapters + 1}`,
    {vlan: adapters_type}
  );

  let add_adapter_request = full_vm.reconfigure(new_config);
  add_adapter_request.wait_for_request({method: "ui"});
  request.addfinalizer(add_adapter_request.remove_request);

  wait_for(
    () => (
      full_vm.configuration.num_network_adapters == new_config.num_network_adapters
    ),
  ...
  let remove_adapter_request = full_vm.reconfigure(orig_config);
  remove_adapter_request.wait_for_request({method: "ui"});
  request.addfinalizer(remove_adapter_request.remove_request);

  wait_for(
    () => (
      full_vm.configuration.num_network_adapters == orig_config.num_network_adapters
    ),
...
//   
// pass
function test_reconfigure_vm_vmware_mem_multiple() {};
...
//   
// pass
function test_reconfigure_vm_vmware_sockets_multiple() {};
...
//   
// pass
function test_reconfigure_vm_vmware_cores_multiple() {};
...
//   
// pass
function test_reconfigure_add_disk_cold() {};
...
//   
// pass
function test_reconfigure_add_disk_cold_controller_sas() {};

function vm_reconfig_via_rest(appliance, config_type, vm_id, config_data) {
  let payload = {
    action: "create",
...
function test_vm_disk_reconfig_via_rest(appliance, full_vm) {
  // 
  //   Polarion:
  //       assignee: pvala
  //       casecomponent: Infra
  ...
  //       1692801
  //   
  let vm_id = appliance.rest_api.collections.vms.get({name: full_vm.name}).id;
  let initial_disks = full_vm.configuration.disks.map(disk => disk.filename);
  ...
  if (is_bool(!BZ(1691635).blocks && full_vm.provider.one_of(RHEVMProvider))) {
    let disks_present = full_vm.configuration.disks.map(disk => disk.filename);
    let disk_added = new Set(disks_present) - new Set(initial_disks).to_a[0];
    let delete_data = [{disk_name: disk_added, delete_backing: false}];
    ...
    try {
      ...
      if ($EXCEPTION instanceof TimedOutError) {
        if (!false) {
          throw "Number of disks expected was {expected}, found {actual}".format({
            expected: initial_disks.size,
            actual: full_vm.configuration.num_disks
          })
        }
      } else {
        throw $EXCEPTION
...
//   
// pass
function test_vm_reconfigure_from_global_region(context) {}
