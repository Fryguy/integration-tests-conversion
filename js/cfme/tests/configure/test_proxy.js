...
let pytestmark = [
  ...
  pytest.mark.provider(
    [AzureProvider, GCEProvider, EC2Provider],
    {scope: "module"}
  ),
...
function validate_proxy_logs(provider, utility_vm_ssh, appliance_ip) {
  let _is_ip_in_log = () => {
...
function prepare_proxy_specific(utility_vm_ssh, provider, appliance, utility_vm_proxy_data) {
  let [proxy_ip, proxy_port] = utility_vm_proxy_data;
  let prov_type = provider.type;
  appliance.set_proxy("192.0.2.1", proxy_port, {prov_type: "default"});
  appliance.set_proxy(proxy_ip, proxy_port, {prov_type});
...
function prepare_proxy_default(utility_vm_ssh, provider, appliance, utility_vm_proxy_data) {
  let [proxy_ip, proxy_port] = utility_vm_proxy_data;
  let prov_type = provider.type;
...
function prepare_proxy_invalid(provider, appliance) {
  let prov_type = provider.type;
  appliance.set_proxy("192.0.2.1", "1234", {prov_type: "default"});
  appliance.set_proxy("192.0.2.1", "1234", {prov_type});
  yield;
  appliance.reset_proxy(prov_type);
  appliance.reset_proxy()
};

function test_proxy_valid(appliance, utility_vm_ssh, prepare_proxy_default, provider) {
...
function test_proxy_override(appliance, utility_vm_ssh, prepare_proxy_specific, provider) {
...
function test_proxy_invalid(appliance, prepare_proxy_invalid, provider) {
  ...
  let last_refresh_failed = () => {
    view.toolbar.reload.click();
    let refresh_failed_msg = (is_bool(provider.one_of(EC2Provider)) ? "execution expired" : "Timed out connecting to server");
    return view.entities.summary("Status").get_text_of("Last Refresh").include(refresh_failed_msg)
  };

  wait_for(method("last_refresh_failed"), {
...
//   
// pass
function test_proxy_remove_default() {};
...
//   
// pass
function test_proxy_remove_azure() {}
