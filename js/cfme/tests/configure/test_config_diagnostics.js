...
function test_configuration_dropdown_roles_by_server(appliance, request) {
  // 
  //   Polarion:
  //       assignee: pvala
  ...
  if (!view.rolesbyservers.configuration.is_displayed) throw new ();

  if (!view.rolesbyservers.configuration.item_enabled("Suspend Role")) {
    throw new ()
  };

  let log = LogValidator(
    "/var/www/miq/vmdb/log/production.log",
    {failure_patterns: [".*Error caught: .*ActiveRecord::RecordNotFound.* Couldn't find MiqServer with 'id'=.*"]}
  );
  ...
  if (!log.validate({wait: "20s"})) throw new ();

  if (is_bool(BZ(1734393, {forced_streams: ["5.10"]}).blocks)) {
    view.rolesbyservers.tree.select_item("SmartState Analysis")
  };

  if (!view.rolesbyservers.tree.currently_selected_role.include("available")) {
    throw new ()
  }
};

function test_diagnostics_server(appliance, obj) {
  // 
  //   Polarion:
  //       assignee: pvala
  //       casecomponent: Configuration
  ...
  //       1498090
  //   
  let context_obj = (obj == "Region" ? appliance.server.zone.region : appliance.server.zone);
  let required_view = appliance.server.create_view(ServerDiagnosticsView);
  let view = navigate_to(context_obj, "Servers");
  view.servers.table.row({name: appliance.server.name}).click();
  if (!required_view.is_displayed) throw new ();
  if (!required_view.summary.is_active()) throw new ()
}
