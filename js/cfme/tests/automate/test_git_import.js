...
const GIT_REPO_URL = null;

// pass
try {
  const GIT_REPO_URL = cfme_data.automate_links.datastore_repositories.manageiq_automate
} catch ($EXCEPTION) {
...
function imported_domain(appliance) {
  ...
  let repo = appliance.collections.automate_import_exports.instantiate({
    import_type: "git",
    url: GIT_REPO_URL,
    verify_ssl: false
  });

  let domain = repo.import_domain_from({branch: "origin/master"});
  yield(domain);
...
function test_automate_git_domain_removed_from_disk(appliance, imported_domain) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  ...
  let repo_path = urlparse(GIT_REPO_URL).path;

  if (!(appliance.ssh_client.run_command(`[ ! -d \"/var/www/vmdb/data/git_repos${repo_path}\" ]`)).success) {
    throw new ()
  }
};

function test_automate_git_domain_displayed_in_service(appliance) {
  ...
  let imported_domain = repo.import_domain_from({branch: "origin/domain-display"});
  let collection = appliance.collections.catalog_items;
  let cat_item = collection.instantiate(collection.GENERIC, "test");
  ...
  if (view.provisioning_entry_point.value.split_p("/") != [
    "",
    imported_domain.name,
    ...path[_.range(2, 0)]
  ]) throw new ()
...
function test_automate_git_import_multiple_domains(request, appliance) {
  // 
  ...
  pytest.raises(
    ...
    () => {
      let domain = repo.import_domain_from({branch: "origin/multi-domains"});
...
function test_domain_import_git(request, appliance, param_type, param_value, verify_ssl) {
  ...
  let domain = repo.import_domain_from({None: {param_type: param_value}});
  request.addfinalizer(domain.delete);
  if (!domain.exists) throw new ()
};

function test_refresh_git_current_user(imported_domain, new_user) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  //       initialEstimate: 1/8h
  ...
  //       1592428
  //   
  let tasks_collection = imported_domain.appliance.collections.tasks;

  imported_domain.refresh({
    branch_or_tag: "Branch",
    git_branch: "origin/master"
  });

  let view = imported_domain.create_view(DomainDetailsView);
  view.flash.assert_message("Successfully refreshed!");
  view = navigate_to(tasks_collection, "AllTasks");
  let all_tasks = view.tabs.alltasks.table.read();
  let __dummy0__ = false;

  for (let task in all_tasks) {
    if (task["Task Name"] == "Refresh git repository") {
      if (task.User != "admin") throw new ();
      break
    };

    if (task == all_tasks[-1]) __dummy0__ = true
  };

  if (__dummy0__) throw new NameError("Task not found");
  ...
  new_user(() => {
    ...
    for (let task in all_tasks) {
      if (task["Task Name"] == "Refresh git repository") {
        if (task.User != new_user.credential.principal) throw new ();
        break
      };

      if (task == all_tasks[-1]) __dummy1__ = true
    };

    if (__dummy1__) throw new NameError("Task not found")
  })
};

function test_domain_import_git_rest(appliance, request) {
  // 
  ...
  //               Query: {
    ...
    let collection = appliance.rest_api.collections.automate_domains;
    ...
    let automate_domain = collection.get({name: "testdomain"});

    let _cleanup = () => {
      let domain = appliance.collections.domains.instantiate({name: "testdomain"});
      return domain.delete_if_exists()
    };

    if (!automate_domain.exists) throw new ()
  };

  function test_automate_git_import_case_insensitive(request, appliance) {
    // 
  ...
  function test_automate_git_domain_import_connection(request, temp_appliance_preconfig, connection) {
    // 
    //   Bugzilla:
    //       1391208
    // 
    ...
    let repo = temp_appliance_preconfig.collections.automate_import_exports.instantiate({
      import_type: "git",
      url: GIT_REPO_URL,
      verify_ssl: true
    });
    ...
    if (is_bool(connection)) {
      ...
      temp_appliance_preconfig.ssh_client.run_command("echo '8.8.8.8 github.com' >> /etc/hosts");
      let msg = "Error during repository fetch: hostname does not match certificate";
  ...
  function test_automate_git_import_without_master(appliance, request) {
    // 
    //   Bugzilla:
    //       1508881
    // 
    ...
    let domain = repo.import_domain_from({branch: "origin/testbranch"});
    request.addfinalizer(domain.delete_if_exists)
  };

  function setup_datastore(appliance) {
  ...
  function test_automate_git_verify_ssl(appliance, setup_datastore, imported_domain) {
    // 
    //   Bugzilla:
    //       1470738
    // 
    ...
    //           3. It should be 0
    //   
    let repo_table = appliance.db.client.git_repositories;

    let check_verify_ssl = () => {
      let repo = appliance.db.client.session.query(repo_table).first();
      if (repo.verify_ssl != 0) return throw new ()
    };
  ...
  function test_automate_git_import_deleted_tag(appliance, imported_domain) {
    // 
    ...
    //   
    let view = navigate_to(imported_domain, "Refresh");
    view.branch_or_tag.fill("Tag");
    if (view.git_tags.read() != "0.1") throw new ()
  };

  function test_git_refresh_with_renamed_yaml(appliance) {
    // 
    ...
    pytest.raises(
      RuntimeError,
      {match: "Error: import failed: Selected branch or tag does not contain a valid domain"},
