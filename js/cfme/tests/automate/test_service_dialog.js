...
let pytestmark = [test_requirements.dialog, pytest.mark.tier(3)];

function create_dialog(appliance, element_data, { label = null }) {
  ...
  let sd = service_dialog.create({label, description: "my dialog"});
  ...
  let element = box.elements.create({element_data: [element_data]});
  return [sd, element]
};

function test_crud_service_dialog(appliance) {
  // 
  //   Polarion:
  //       assignee: nansari
  ...
  let element_data = {
    ...
    options: {default_text_box: "Default text"}
  };

  let [dialog, element] = create_dialog(appliance, element_data);
  let view = appliance.browser.create_view(DialogsView, {wait: "10s"});
  let flash_message = `${dialog.label} was saved`;
  view.flash.assert_message(flash_message);
  update(dialog, () => dialog.description = "my edited description");
  view.flash.assert_message(flash_message);
  dialog.delete()
};

function test_service_dialog_duplicate_name(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
  ...
  let label = fauxfactory.gen_alphanumeric(15, {start: "duplicate_"});

  let [dialog, element] = create_dialog(
    appliance,
    element_data,
    {label}
  );
  ...
  let region_number = appliance.server.zone.region.number;
  let d = (appliance.version < "5.10" ? "" : "Dialog: ");

  let error_message = ("There was an error editing this dialog: Failed to create a new dialog - Validation failed: {d}Name is not unique within region {reg_num}").format({
    d,
...
function test_checkbox_dialog_element(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
  ...
  let element_data = {
    ...
    options: {default_value: true, field_required: true}
  };
...
function test_datecontrol_dialog_element(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
...
function test_tagcontrol_dialog_element(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
  ...
  let element_data = {
    ...
    options: {field_category: "Service Level", field_required: "Yes"}
  };
...
function test_textareabox_dialog_element(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
...
function test_reorder_elements(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
  ...
  //       tags: service
  //   
  let element_1_data = {element_information: {
    ele_label: fauxfactory.gen_alphanumeric(15, {start: "ele_label_"}),
    ele_name: fauxfactory.gen_alphanumeric(15, {start: "ele_name_"}),
    ele_desc: fauxfactory.gen_alphanumeric(15, {start: "ele_desc_"}),
    choose_type: "Text Box"
  ...
  let element_2_data = {element_information: {
    ele_label: fauxfactory.gen_alphanumeric(15, {start: "ele_label_"}),
    ele_name: fauxfactory.gen_alphanumeric(15, {start: "ele_name_"}),
    ele_desc: fauxfactory.gen_alphanumeric(15, {start: "ele_desc_"}),
    choose_type: "Check Box"
  ...
  let sd = service_dialog.create({
    label: fauxfactory.gen_alphanumeric({start: "label_"}),
  ...
  let element = box.elements.create({element_data: [
    element_1_data,
    element_2_data
  ]});
...
function test_reorder_unsaved_elements(appliance, request) {
  // 
  //   Bugzilla:
  //       1238721
  // 
  ...
  //       tags: service
  //   
  let box_label = fauxfactory.gen_alphanumeric({start: "box_"});
  ...
  let box = tab.boxes.create({box_label, box_desc: "my box desc"});
  let element = box.elements.create({element_data: [element_1_data]});
  request.addfinalizer(sd.delete_if_exists);
  element.reorder_elements(true, element_2_data, element_1_data)
};

function test_dropdownlist_dialog_element(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
...
function test_radiobutton_dialog_element(appliance, request) {
  // 
  //   Polarion:
  //       assignee: nansari
  //       casecomponent: Services
...
function test_mandatory_entry_point_with_dynamic_element(appliance) {
  ...
  if (box.elements.create({element_data: [element_1_data]}) !== false) {
    throw new ()
  };

  let view_cls = navigator.get_class(sd.parent, "Add").VIEW;
  let view = appliance.browser.create_view(view_cls);
  if (!view.save.disabled) throw new ()
};
...
//   
// pass
function test_default_value_on_dropdown_inside_dialog() {};
...
//   
// pass
function test_dialog_items_default_values_on_different_screens() {};

function test_automate_methods_from_dynamic_dialog_should_run_as_per_designed(request, appliance, import_datastore, import_data, import_dialog, catalog, soft_assert) {
  // 
  //   Bugzilla:
  //       1571000
  // 
  ...
  for (let pattern in patterns) {
    soft_assert.call(result.matches[pattern] == 1)
  };
  ...
  (LogValidator(
    ...
    for (let ele_name in [ele_label, "label2", "label3"]) {
      view.fields(ele_name).input.fill(fauxfactory.gen_alphanumeric())
    }
  })
};

function test_service_dialogs_crud_non_admin_user(appliance, user_self_service_role) {
  // 
  //   Bugzilla:
  //       1677724
  // 
...
function custom_button(appliance, import_dialog) {
  // This fixture creates custom button for user object
  let [sd, ele_label] = import_dialog;
  let collection = appliance.collections.button_groups;

  let button_grp = collection.create({
    text: fauxfactory.gen_alphanumeric(),
    hover: fauxfactory.gen_alphanumeric(),
    type: collection.getattr("USER")
  });

  let button = button_grp.buttons.create({
    text: fauxfactory.gen_alphanumeric(),
    hover: fauxfactory.gen_alphanumeric(),
    dialog: sd,
    system: "Request",
...
function navigation_to_view(custom_button_group, button) {
  // This function helps to navigate to view that comes after clicking on custom button
  custom_button_group.item_select(button.text);
  let view = button.create_view(TextInputAutomateView, {wait: "10s"});
  return view
};

function test_dialog_element_values_passed_to_button(appliance, import_datastore, import_data, custom_button, file_name) {
  // 
  //   Bugzilla:
  //       1715396
  //       1717501
  ...
  //   
  let [button_grp, button] = custom_button;
  let user_obj = appliance.collections.users.instantiate({name: "Administrator"});
  let view = navigate_to(user_obj, "Details");
  let custom_button_group = Dropdown(view, button_grp.hover);
  ...
  (LogValidator(
    ...
    {matched_patterns: [
      ".*dialog_text_box_1:      .*",
      `.*dialog_text_box_2: ${view.text_box2.read()}.*`
    ]}
  )).waiting({timeout: 120}, () => view.submit.click());
  ...
  (LogValidator(
    ...
    {matched_patterns: [
      `.*dialog_text_box_1: ${msg}.*`,
      `.*dialog_text_box_2: also${view.text_box2.read()}.*`
    ]}
  )).waiting({timeout: 120}, () => {
    view.text_box1.fill(msg);
