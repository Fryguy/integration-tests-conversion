...
function test_instance_crud(klass) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  ...
  let orig = instance.description;
  update(instance, () => instance.description = "edited");
  update(instance, () => instance.description = orig);
  instance.delete();
  if (!!instance.exists) throw new ()
};

function test_duplicate_instance_disallowed(klass) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  //       casecomponent: Automate
...
function test_instance_display_name_unset_from_ui(klass) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  //       casecomponent: Automate
  ...
  if (!instance.exists) throw new ();
  update(instance, () => instance.display_name = "");
  if (!instance.exists) throw new ()
};

function test_automate_instance_missing(domain, klass, namespace, appliance) {
  ...
  //       tags: automate
  //   
  let catch_string = fauxfactory.gen_alphanumeric();

  let method = klass.methods.create({
    name: fauxfactory.gen_alphanumeric(),
    location: "inline",
    script: `$evm.log(:info, \"${catch_string}\")`
  });
  ...
  klass.instances.create({
    name: ".missing",
    fields: {mfield: {value: "${#_missing_instance}"}}
  });

  let klass2 = namespace.classes.create({name: fauxfactory.gen_alpha()});
  klass2.schema.add_fields({name: "rel", type: "Relationship"});

  let instance2 = klass2.instances.create({
    name: fauxfactory.gen_alphanumeric(),

    fields: {rel: {value: ("/") + (method.tree_path_name_only[_.range(
      1,
      0
    )].join("/"))}}
  });
  ...
  if (!(appliance.ssh_client.run_command(`grep ${catch_string} /var/www/miq/vmdb/log/automation.log`)).success) {
    throw new ()
  }
};

function test_automate_relationship_trailing_spaces(request, klass, namespace, domain) {
  // 
  ...
  let klass2 = namespace.classes.create({
  ...
  let instance2 = klass2.instances.create({
    ...
    fields: {rel: {value: ("/{domain}/{namespace}/{klass}/{instance}   ").format({
      domain: domain.name,
...
function copy_instance(domain) {
  // 
  //   This fixture copies the instance '/ManageIQ/System/Request/ansible_tower_job' to new domain.
  //   
  let klass = domain.parent.instantiate({name: "ManageIQ"}).namespaces.instantiate({name: "System"}).classes.instantiate({name: "Request"});
  klass.instances.instantiate({name: "ansible_tower_job"}).copy_to(domain.name);
  let instance = domain.namespaces.instantiate({name: "System"}).classes.instantiate({name: "Request"}).instances.instantiate({name: "ansible_tower_job"});
  yield(instance)
};

function test_check_system_request_calls_depr_conf_mgmt(appliance, copy_instance) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  ...
  //       1615444
  //   
  let search = "/AutomationManagement/AnsibleTower/Operations/StateMachines/Job/default";

  let result = LogValidator(
    "/var/www/miq/vmdb/log/automation.log",
    {matched_patterns: [`.*${search}.*`]}
  );

  result.start_monitoring();
  simulate({appliance, request: copy_instance.name});
  if (!result.validate({wait: "60s"})) throw new ()
};

function copy_quota_instance(domain) {
  // Copy the default instance 'quota' to custom domain
  let miq = domain.appliance.collections.domains.instantiate("ManageIQ");
  let original_instance = miq.namespaces.instantiate("System").namespaces.instantiate("CommonMethods").classes.instantiate("QuotaStateMachine").instances.instantiate("quota");
...
function test_quota_source_value(request, entity, search, copy_quota_instance, generic_catalog_item) {
  // 
  //   Polarion:
  //       assignee: dgaikwad
  //       initialEstimate: 1/8h
  ...
  let root_tenant = copy_quota_instance.appliance.collections.tenants.get_root_tenant();
  root_tenant.set_quota({None: {cpu_cb: true, cpu: 3}});
  request.addfinalizer(() => root_tenant.set_quota({None: {cpu_cb: false}}));
  ...
  let request_description = "Provisioning Service [{name}] from [{name}]".format({name: service_catalogs.name});
  let provision_request = copy_quota_instance.appliance.collections.requests.instantiate({description: request_description});
  service_catalogs.order();
  provision_request.wait_for_request({method: "ui"});
  request.addfinalizer(() => provision_request.remove_request({method: "rest"}));
  if (!result.validate({wait: "60s"})) throw new ()
};

function setup(domain, klass, namespace) {
  ...
  let state1 = fauxfactory.gen_alpha();
  let state2 = fauxfactory.gen_alpha();
  let state3 = fauxfactory.gen_alpha();

  klass.schema.add_fields(...[state1, state2, state3].map(state => ({
    name: state,
    type: "State"
  })));
...
exit MIQ_STOP
  ...
  let instance = klass.instances.create({
    ...
    display_name: fauxfactory.gen_alphanumeric(),
    description: fauxfactory.gen_alphanumeric(),
    fields: {state3: {value: `METHOD::${method.name}`}}
  });
  ...
  klass2.schema.add_fields(...[state1, state2].map(state => ({
    name: state,
    type: "State"
  })));
...
function test_miq_stop_abort_with_state_machines(request, setup, process, domain, klass, namespace) {
  // 
  //   Bugzilla:
  //       1441353
  // 
  ...
  //             method A1 - \"Hello from method of parent instance\"
  //   
  let [klass2, instance, state1, state2, state3] = setup;

  let child_method = ["first", "second", "third"].map(num => (
    klass2.methods.create({
      name: fauxfactory.gen_alphanumeric(),
      display_name: fauxfactory.gen_alphanumeric(),
      location: "inline",
      script: `
...
exit ${process.upcase()}
  ...
  let fields = [
    {
      state1: {value: `METHOD::${child_method[0].name}`},
      state2: {value: `METHOD::${child_method[1].name}`}
    },

    {state1: {value: `METHOD::${child_method[2].name}`}}
  ];

  let child_inst = fields.map(field => (
    klass2.instances.create({
      name: fauxfactory.gen_alphanumeric(),
      display_name: fauxfactory.gen_alphanumeric(),
      description: fauxfactory.gen_alphanumeric(),
      fields: field
  ...
  let finalize = () => {
    for (let method in child_method) {
      method.delete_if_exists()
    };

    for (let inst in child_inst) {
  ...
  update(instance, () => (
    instance.fields = {
      state1: {value: `/${domain.name}/${namespace.name}/${klass2.name}/${child_inst[0].name}`},
      state2: {value: `/${domain.name}/${namespace.name}/${klass2.name}/${child_inst[1].name}`}
    }
  ));
