...
const TEST_ITEMS = [
  ContainersTestItem(
    ContainersProvider,
    "container_provider_table_fields",
  ...
  ContainersTestItem(Route, "route__table_fields", {
    fields_to_verify: ["provider", "project_name"],
    collection_name: "container_routes"
  }),

  ContainersTestItem(Container, "container__table_fields", {
    fields_to_verify: ["pod_name", "image", "state"],
    collection_name: "containers"
  }),

  ContainersTestItem(Pod, "pod__table_fields", {
    fields_to_verify: [
      "provider",
      "project_name",
      "ready",
  ...
  ContainersTestItem(Service, "service_table_fields9", {
    fields_to_verify: [
      "provider",
      "project_name",
      "type",
  ...
  ContainersTestItem(Node, "node_table_fields", {
    fields_to_verify: [
      "provider",
      "ready",
      "operating_system",
  ...
  ContainersTestItem(Replicator, "replicator_table_fields", {
    fields_to_verify: [
      "provider",
      "project_name",
      "replicas",
...
function test_tables_fields(provider, test_item, soft_assert, appliance) {
  // 
  //   Polarion:
  //       assignee: juwatts
  //       caseimportance: high
  ...
  for (let row in view.entities.elements.rows()) {
    let name_field = row.getattr("name", row.getattr("host", null));
    let name = name_field.text;

    for (let field in test_item.fields_to_verify) {
      try {
        let value = row.getattr(field)
      } catch ($EXCEPTION) {
        if ($EXCEPTION instanceof NoMethodError) {
          soft_assert.call(
            false,

            "{}'s list table: field  not exist: {}".format(
      ...
      soft_assert.call(
        ...
        ("{}'s list table: {} row - has empty field: {}").format(
          test_item.obj.__name__,
          name,
          field
        )
...
function test_containers_details_view_title(appliance) {
  // 
  ...
  //       initialEstimate: 1/6h
  //   
  let random_container = appliance.collections.containers.all().pop();
  let view = navigate_to(random_container, "Details");

  if (!view.title.text.include("Summary")) {
    throw "The word \"Summary\" is missing in container details view"
  }
}
