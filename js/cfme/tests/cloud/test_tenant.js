...
function tenant(provider, setup_provider, appliance) {
  let tenant = appliance.collections.cloud_tenants.create({
...
function test_tenant_crud(tenant) {
...
function child_tenant(new_tenant) {
  // This fixture used to create child tenant under parent tenant - new_tenant
  let child_tenant = new_tenant.appliance.collections.tenants.create({
    name: TENANTS[1],
...
function check_permissions(appliance, assigned_tenant) {
  ...
  if (!!view.toolbar.configuration.has_item("Manage Quotas")) throw new ()
};

function test_dynamic_product_feature_for_tenant_quota(request, appliance, new_tenant, child_tenant) {
  // 
  //   Polarion:
  //       assignee: tpapaioa
  ...
  let user_ = [];
  let role_ = [];
  let product_feature = ["Everything"];

  product_feature.concat((appliance.version < "5.11" ? [
    "Settings",
    "Configuration"
  ] : ["Main Configuration"]));

  product_feature.concat([
    "Access Control",
    "Tenants",
    "Modify",
    "Manage Quotas"
  ...
  let tenant_ = [
    `My Company/${new_tenant.name}`,

    ("My Company/{parent}/{child}").format({
      parent: new_tenant.name,
      child: child_tenant.name
    })
  ];

  let role = appliance.collections.roles.instantiate({name: "EvmRole-tenant_administrator"});

  for (let i in (2).times) {
    let new_role = role.copy({name: "{name}_{role}".format({
      name: role.name,
  ...
  user_[0, () => (
    check_permissions({appliance, assigned_tenant: new_tenant.name})
  )];

  user_[1, () => (
    check_permissions({appliance, assigned_tenant: child_tenant.name})
  )]
};

function test_tenant_quota_input_validate(appliance) {
  // 
  //   Polarion:
  //       assignee: tpapaioa
  //       casecomponent: WebUI
  ...
  for (let field in fields) {
    let view = navigate_to(roottenant, "ManageQuotas");

    view.form.fill({
      ["{}_cb".format(field[0])]: true,
      ["{}_txt".format(field[0])]: field[1]
    });
