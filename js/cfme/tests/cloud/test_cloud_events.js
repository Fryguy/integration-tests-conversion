...
let pytestmark = [
  ...
  pytest.mark.provider(
    [AzureProvider],
    {scope: "module", required_flags: ["events"]}
  ),
...
function test_manage_nsg_group(appliance, provider, register_event) {
  // 
  ...
  //   
  let nsg_name = random_vm_name({context: "nsg"});
  let resource_group = provider.data.provisioning.resource_group;
  ...
  let fd_add_attr = {full_data: "will be ignored", cmp_func: add_cmp};

  register_event(fd_add_attr, {
    source: provider.type.upcase(),
  ...
  let fd_rm_attr = {full_data: "will be ignored", cmp_func: rm_cmp};
...
function test_vm_capture(appliance, request, create_vm, provider, register_event) {
  // 
  ...
  register_event(
    full_data_attr,
    {source: "AZURE", event_type: "virtualMachines_capture_EndRequest"}
  );

  create_vm.mgmt.capture({
    container: "templates",
    image_name: create_vm.name
  });

  let blob_images = provider.mgmt.find_templates({
    container: "system",
    name: create_vm.name,
    only_vhd: false
  });
