...
pytestmark = [test_requirements.configuration, pytest.mark.rhel_testing, pytest.mark.tier(3)]
def test_send_test_email(smtp_test, random_string, appliance)
  ...
  wait_for(lambda{|| smtp_test.get_emails(to_address: e_mail).size > 0}, num_sec: 60)
end
