...
require_relative 'cfme/utils/version'
include Cfme::Utils::Version
pytestmark = [test_requirements.restore]
...
def get_ext_appliances_with_providers(temp_appliances_unconfig_funcscope_rhevm, app_creds_modscope)
  ...
  appl1.wait_for_web_ui()
  appl1.wait_for_api_available()
  appl2.appliance_console_cli.configure_appliance_external_join(app_ip, app_creds_modscope["username"], app_creds_modscope["password"], "vmdb_production", app_ip, app_creds_modscope["sshlogin"], app_creds_modscope["sshpass"])
...
def get_ha_appliances_with_providers(unconfigured_appliances, app_creds)
  ...
  Wait_for::wait_for(lambda{|| appl1.db.is_dedicated_active})
  command_set = ["ap", "", "7", "2", app0_ip, "", pwd, "", "2", "0", "y", app0_ip, "", "", "", TimedCommand(pwd, 360), ""]
  ...
  appl1.appliance_console.run_commands(command_set)
  command_set = ["ap", "", "8", "2", "2", app0_ip, "", pwd, "", "2", "2", "", "", pwd, pwd, app0_ip, app1_ip, "y", TimedCommand("y", 60), ""]
  appl2.appliance_console.run_commands(command_set)
  waiting_for_ha_monitor_started(appl3, app1_ip, timeout: 300) {
...
def restore_db(appl, location: "")
  SSHExpect(appl) {|interaction|
    ...
    interaction.answer("Choose the advanced setting: ", VersionPicker({"LOWEST" => "6", "5.11.2.1" => 4}))
    interaction.answer(re.escape("Choose the restore database file source: |1| "), "1")
    interaction.answer(re.escape("Enter the location of the local restore file: |/tmp/evm_db.backup| "), location)
    interaction.answer(re.escape("Should this file be deleted after completing the restore? (Y/N): "), "N")
    interaction.answer(re.escape("Are you sure you would like to restore the database? (Y/N): "), "Y")
...
def test_appliance_console_dump_restore_db_local(request, get_appliances_with_providers)
  ...
  raise "Restored DB is missing some providers" unless Set.new(appl2.managed_provider_names) == Set.new(appl1.managed_provider_names)
  virtual_crud = provider_app_crud(VMwareProvider, appl2)
  vm = provision_vm(request, virtual_crud)
  raise "vm not running" unless vm.mgmt.is_running
end
def test_appliance_console_backup_restore_db_local(request, two_appliances_one_with_providers)
  ...
  SSHExpect(appl2) {|interaction|
    ...
    interaction.answer("Press any key to continue.", "", timeout: 40)
    interaction.answer("Choose the advanced setting: ", VersionPicker({"LOWEST" => "6", "5.11.2.1" => 4}))
    interaction.answer(re.escape("Choose the restore database file source: |1| "), "")
    interaction.answer(re.escape("Enter the location of the local restore file: |/tmp/evm_db.backup| "), backup_file_name)
    interaction.answer(re.escape("Should this file be deleted after completing the restore? (Y/N): "), "n")
    interaction.answer(re.escape("Are you sure you would like to restore the database? (Y/N): "), "y")
  ...
  appl2.wait_for_web_ui()
  appl2.wait_for_api_available()
  raise "Restored DB is missing some providers" unless Set.new(appl2.managed_provider_names) == appl1_provider_names
  virtual_crud = provider_app_crud(VMwareProvider, appl2)
  vm = provision_vm(request, virtual_crud)
  raise "vm not running" unless vm.mgmt.is_running
end
def test_appliance_console_restore_pg_basebackup_ansible(get_appliance_with_ansible)
  ...
  begin
    repository = repositories.create(name: "example", url: cfme_data.ansible_links.playbook_repositories.console_db, description: "example")
  rescue KeyError
    pytest.skip("Skipping since no such key found in yaml")
  end
  view = navigate_to(repository, "Details")
  refresh = view.toolbar.refresh.click
  Wait_for::wait_for(lambda{|| view.entities.summary("Properties").get_text_of("Status") == "successful"}, timeout: 60, fail_func: refresh, message: "Check if playbook repo added")
end
def test_appliance_console_restore_pg_basebackup_replicated(request, replicated_appliances_with_providers)
  # 
  #   Polarion:
  #       assignee: jhenner
  #       casecomponent: Configuration
  ...
  appl1.db.backup()
  appl2.db.backup()
  providers_before_restore = Set.new(appl1.managed_provider_names)
  ...
  appl1.wait_for_api_available()
  appl2.wait_for_api_available()
  raise "Restored DB is missing some providers" unless providers_before_restore == Set.new(appl1.managed_provider_names)
  raise "Restored DB is missing some providers" unless providers_before_restore == Set.new(appl2.managed_provider_names)
  ...
  raise "vm not running" unless vm1.mgmt.is_running
  raise "vm not running" unless vm2.mgmt.is_running
end
...
def test_appliance_console_restore_db_replicated(request, replicated_appliances_with_providers)
  # 
  #   Polarion:
  #       assignee: jhenner
  #       caseimportance: high
  ...
  appl2.wait_for_api_available()
  appl2.set_pglogical_replication(replication_type: ":none")
  expected_providers = (appl2.version < "5.11") ? [] : ["Embedded Ansible"]
  raise unless appl2.managed_provider_names == expected_providers
  appl2.set_pglogical_replication(replication_type: ":global")
  appl2.add_pglogical_replication_subscription(appl1.hostname)
  raise unless providers_before_restore == Set.new(appl1.managed_provider_names)
  Wait_for::wait_for(lambda{|| providers_before_restore == Set.new(appl2.managed_provider_names)}, timeout: 20)
  virtual_crud_appl1 = provider_app_crud(VMwareProvider, appl1)
  virtual_crud_appl2 = provider_app_crud(VMwareProvider, appl2)
  vm1 = provision_vm(request, virtual_crud_appl1)
  vm2 = provision_vm(request, virtual_crud_appl2)
...
def test_appliance_console_restore_db_ha(request, unconfigured_appliances, app_creds)
  ...
  provider_app_crud(OpenStackProvider, appl3).setup()
  appl1.db.backup()
  providers_before_restore = Set.new(appl3.managed_provider_names)
  ...
  raise "Restored DB is missing some providers" unless providers_before_restore == Set.new(appl3.managed_provider_names)
  LogValidator(evm_log, matched_patterns: ["Starting to execute failover"], hostname: appl3.hostname).waiting(timeout: 450) {
...
def test_appliance_console_restore_db_nfs(request, two_appliances_one_with_providers, utility_vm, utility_vm_nfs_ip)
  ...
  SSHExpect(appl1) {|interaction|
    ...
    interaction.send("ap")
    interaction.answer("Press any key to continue.", "", timeout: 40)
    interaction.answer("Choose the advanced setting: ", VersionPicker({"LOWEST" => "4", "5.11.2.1" => 2}))
    interaction.answer("Choose the backup output file destination: \\|1\\| ", "2")
    interaction.answer("Enter the location to save the backup file to: \\|.*\\| ", nfs_dump_file_name)
    interaction.answer(re.escape("Example: nfs://host.mydomain.com/exported/my_exported_folder/db.backup: "), nfs_restore_dir_path)
  ...
  SSHExpect(appl2) {|interaction|
    ...
    interaction.answer("Press any key to continue.", "", timeout: 40)
    interaction.answer("Choose the advanced setting: ", VersionPicker({"LOWEST" => "6", "5.11.2.1" => 4}))
    interaction.answer("Choose the restore database file source: \\|1\\| ", "2")
    interaction.answer(re.escape("Example: nfs://host.mydomain.com/exported/my_exported_folder/db.backup: "), nfs_restore_file_path)
...
def test_appliance_console_restore_db_samba(request, two_appliances_one_with_providers, utility_vm, utility_vm_samba_ip)
  ...
  SSHExpect(appl1) {|interaction|
    ...
    interaction.answer("Press any key to continue.", "", timeout: 40)
    interaction.answer("Choose the advanced setting: ", VersionPicker({"LOWEST" => "4", "5.11.2.1" => 2}))
    interaction.answer("Choose the backup output file destination: \\|1\\| ", "3")
