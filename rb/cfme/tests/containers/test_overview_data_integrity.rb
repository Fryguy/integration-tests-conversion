...
tested_objects = [ImageRegistry, Container, Project, Pod, Service, Route, ContainersProvider, Node]
def get_api_object_counts(appliance)
  out = {"ContainersProvider" => 0, "Container" => 0, "ImageRegistry" => 0, "Node" => 0, "Pod" => 0, "Service" => 0, "Project" => 0, "Route" => 0}
...
def test_containers_overview_data_integrity(appliance, soft_assert)
  ...
  for cls in tested_objects
    statusbox_value = (view.status_cards(cls.PLURAL.split_p(" ")[-1])).value
    soft_assert.(api_values[cls] == statusbox_value, "There is a mismatch between API and UI values: {}: {} (API) != {} (UI)".format(cls.__name__, api_values[cls], statusbox_value))
  end
end
