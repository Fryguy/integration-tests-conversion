...
pytestmark = [pytest.mark.provider(classes: [RHEVMProvider], selector: ONE_PER_TYPE, scope: "module"), pytest.mark.provider(classes: [VMwareProvider], selector: ONE_PER_TYPE, fixture_name: "source_provider", scope: "module")]
def test_show_quota_used_on_tenant_screen(appliance, v2v_provider_setup)
  ...
  vm_count = v2v_provider_setup.rhv_provider.num_vm() + v2v_provider_setup.vmware_provider.num_vm()
  ...
  num_of_vms = num_of_vms.split()[0].to_i
  raise unless vm_count == num_of_vms
end
