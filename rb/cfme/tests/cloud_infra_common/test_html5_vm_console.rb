...
def test_html5_console_ports_present(appliance, setup_provider, provider)
  ...
  raise unless edit_view.vnc_start_port.is_displayed
  raise unless edit_view.vnc_end_port.is_displayed
end
def test_html5_vm_console(appliance, setup_provider, provider, configure_websocket, create_vm, configure_console_vnc, take_screenshot)
  # 
  ...
  raise "VMConsole object should be created" unless create_vm.vm_console
  ...
  begin
    ...
    if is_bool(provider.one_of(VMwareProvider))
      raise "Wrong console type: looking for VNC found {}".format(vm_console.console_type) unless vm_console.console_type == "VNC"
    end
    Wait_for::wait_for(func: lambda{|| vm_console.get_screen_text() != ""}, delay: 5, timeout: 45)
    ...
    if is_bool(!provider.one_of(OpenStackProvider))
      raise "VM Console Toggle Full Screen button doesn't work" unless vm_console.send_fullscreen()
    end
  rescue Exception
