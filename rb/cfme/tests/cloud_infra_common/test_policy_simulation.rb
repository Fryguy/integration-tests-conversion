...
pytestmark = [pytest.mark.usefixtures("setup_provider"), pytest.mark.tier(2), pytest.mark.provider(classes: [CloudProvider, InfraProvider], selector: ONE_PER_CATEGORY, scope: "module"), test_requirements.control]
def test_policy_simulation_ui(provider, navigation)
  ...
  raise unless view.form.entities.check_context_against_entities([vm])
  raise unless view.form.cancel_button.is_displayed
  view.form.policy_profile.select_by_visible_text("OpenSCAP profile")
  if is_bool(vm.provider.one_of(InfraProvider) || !BZ(1688359, forced_streams: ["5.10"]).blocks)
