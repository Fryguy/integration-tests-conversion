...
pytestmark = [pytest.mark.long_running, pytest.mark.meta(server_roles: ["+automate", "+smartproxy", "+notifier"]), pytest.mark.provider(CANDU_PROVIDER_TYPES, scope: "module"), pytest.mark.usefixtures("setup_provider_modscope"), pytest.mark.tier(3), test_requirements.alert]
def wait_for_alert(smtp, alert, delay: nil, additional_checks: nil)
    ...
    for mail in smtp.get_emails()
      if mail["subject"].include?("Alert Triggered: #{alert.description}")
        if is_bool(!additional_checks)
          ...
          for (key, value) in additional_checks.to_a()
            if mail.get(key, "").include?(value)
              return true
            end
          end
        end
  ...
  wait_for(method(:_mail_arrived), num_sec: delay, delay: 5, message: "wait for e-mail to come!")
end
def setup_for_alerts(appliance)
  ...
  #   
  _setup_for_alerts = lambda do |request, alerts_list, event: nil, vm_name: nil, provider: nil|
...
def setup_candu(appliance)
  ...
  yield
  appliance.server.settings.disable_server_roles("ems_metrics_coordinator", "ems_metrics_collector", "ems_metrics_processor")
  candu.disable_all()
end
...
def test_alert_vm_turned_on_more_than_twice_in_past_15_minutes(request, appliance, provider, create_vm, smtp_test, setup_for_alerts)
...
def test_alert_rtp(request, appliance, create_vm, smtp_test, provider, setup_candu, wait_candu, setup_for_alerts)
  ...
  alert = appliance.collections.alerts.create(fauxfactory.gen_alpha(length: 20, start: "Trigger by CPU "), active: true, based_on: "VM and Instance", evaluate: ["Real Time Performance", {"performance_field" => "CPU - % Used", "performance_field_operator" => ">", "performance_field_value" => "0", "performance_trend" => "Don't Care", "performance_time_threshold" => "3 Minutes"}], notification_frequency: "5 Minutes", emails: email)
  request.addfinalizer(alert.delete)
  setup_for_alerts.(request, [alert])
  wait_for_alert(smtp_test, alert, delay: 30 * 60, additional_checks: {"text" => create_vm.name, "from_address" => email})
end
def test_alert_timeline_cpu(request, appliance, create_vm, set_performance_capture_threshold, provider, ssh, setup_candu, wait_candu, setup_for_alerts)
  ...
  #       initialEstimate: 1/6h
  #   
  alert = appliance.collections.alerts.create(fauxfactory.gen_alpha(length: 20, start: "TL event by CPU "), active: true, based_on: "VM and Instance", evaluate: ["Real Time Performance", {"performance_field" => "CPU - % Used", "performance_field_operator" => ">=", "performance_field_value" => "10", "performance_trend" => "Don't Care", "performance_time_threshold" => "2 Minutes"}], notification_frequency: "1 Minute", timeline_event: true)
  request.addfinalizer(alert.delete)
  ...
  timeline.filter.fill({"event_category" => "Alarm/Status Change/Errors", "time_range" => "Weeks", "calendar" => ("{dt.month}/{dt.day}/{dt.year}").format(dt: Datetime::now() + timedelta(days: 1))})
  timeline.filter.apply.click()
  events = timeline.chart.get_events()
  __dummy0__ = false
  for event in events
    if event.message.include?(alert.description)
      break
    end
    if event == events[-1]
      __dummy0__ = true
...
def test_alert_snmp(request, appliance, provider, setup_snmp, setup_candu, create_vm, wait_candu, setup_for_alerts)
  ...
  wait_for(method(:_snmp_arrived), timeout: "30m", delay: 60, message: "SNMP trap arrived.")
end
def test_alert_hardware_reconfigured(request, appliance, configure_fleecing, smtp_test, create_vm, setup_for_alerts)
  ...
  vm = create_vm
  email = fauxfactory.gen_email()
  service_request_desc = "VM Reconfigure for: {0} - Processor Sockets: {1}, Processor Cores Per Socket: 1, Total Processors: {1}"
  alert = appliance.collections.alerts.create(fauxfactory.gen_alpha(length: 36, start: "Trigger by hardware reconfigured "), active: true, based_on: "VM and Instance", evaluate: ["Hardware Reconfigured", {"hardware_attribute" => "Number of CPU Cores", "operator" => "Increased"}], notification_frequency: "1 Minute", emails: email)
  request.addfinalizer(alert.delete)
  ...
  for i in 1.upto(3-1)
    ...
    vm.reconfigure(changes: {"cpu" => true, "sockets" => (sockets_count + i).to_s, "disks" => [], "network_adapters" => []})
    service_request = appliance.collections.requests.instantiate(description: service_request_desc.format(vm.name, sockets_count + i))
    service_request.wait_for_request(method: "ui", num_sec: 300, delay: 10)
  end
  wait_for_alert(smtp_test, alert, delay: 30 * 60, additional_checks: {"text" => vm.name, "from_address" => email})
end
