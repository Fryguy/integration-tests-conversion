...
pytestmark = [pytest.mark.tier(2), pytest.mark.provider([InfraProvider], selector: ONE_PER_TYPE), pytest.mark.usefixtures("setup_provider"), pytest.mark.ignore_stream("5.11"), test_requirements.report]
IDS = ["policy-events-week", "policy-events-7days", "host-mgmt-week", "vm-operations"]
PATHS = [["Events", "Policy", "Policy Events for Last Week", "Policy Events for Last Week"], ["Events", "Policy", "Policy Events for the Last 7 Days", "Policy Events for the Last 7 Days"], ["Configuration Management", "Hosts", "Date brought under Management for Last Week", "Hosts: Date brought under Management for Last Week"], ["Events", "Operations", "Operations VMs Powered On/Off for Last Week", "Operations VM Power On/Off Events for Last Week"]]
UPDATES = [{"filter" => {"primary_filter" => "fill_field(Policy Event : Activity Sample - Timestamp (Day/Time), IS, This Week)"}}, {"report_fields" => ["Activity Sample - Timestamp (Day/Time)", "Miq Policy Sets : Date Created", "Miq Policy Sets : Date Updated", "Miq Policy Sets : Description", "Miq Policy Sets : EVM Unique ID (Guid)"], "filter" => {"primary_filter" => "fill_field(Policy Event : Activity Sample - Timestamp (Day/Time), IS, This Week)"}}, {"filter" => {"primary_filter" => "fill_field(Host / Node : Date Created, IS, This Week)"}}, {"filter" => {"primary_filter" => "fill_field(Event Stream.VM and Instance : Date Created, IS, Today)"}}]
...
def generate_policy_event(request, appliance, provider, vm_crud, register_event)
  ...
  policy = appliance.collections.policies.create(policies.VMControlPolicy, fauxfactory.gen_alpha())
...
def test_reports_with_timelines(appliance, setup_for_reports)
  ...
  raise unless view.chart.get_events().size > 0
end
