...
def test_assign_compute_enterprise(appliance, virtualcenter_provider)
  ...
  enterprise = cb.ComputeAssign(assign_to: "The Enterprise", selections: {"Enterprise" => {"Rate" => "Default"}})
  enterprise.assign()
  ...
  raise "Selection does not match" unless selected_option == "Default"
end
def test_assign_compute_provider(appliance, virtualcenter_provider)
  ...
  #   
  view = navigate_to(appliance.server, "Chargeback")
  compute_provider = cb.ComputeAssign(assign_to: "Selected Providers", selections: {virtualcenter_provider.name => {"Rate" => "Default"}})
...
def test_assign_compute_cluster(appliance, virtualcenter_provider)
  ...
  #   
  view = navigate_to(appliance.server, "Chargeback")
  cluster_name = ("{}/{}").format(virtualcenter_provider.name, random.choice(virtualcenter_provider.data["clusters"]))
  cluster = cb.ComputeAssign(assign_to: "Selected Cluster / Deployment Roles", selections: {"cluster_name" => {"Rate" => "Default"}})
  cluster.assign()
  assign_view = view.browser.create_view(AssignmentsView)
...
def test_assign_compute_taggedvm(appliance, virtualcenter_provider)
  ...
  #   
  view = navigate_to(appliance.server, "Chargeback")
  tagged_vm = cb.ComputeAssign(assign_to: "Tagged VMs and Instances", tag_category: "Location", selections: {"Chicago" => {"Rate" => "Default"}})
  tagged_vm.assign()
  assign_view = view.browser.create_view(AssignmentsView)
...
def test_assign_storage_enterprise(appliance, virtualcenter_provider)
  ...
  #   
  view = navigate_to(appliance.server, "Chargeback")
  enterprise = cb.StorageAssign(assign_to: "The Enterprise", selections: {"Enterprise" => {"Rate" => "Default"}})
  enterprise.assign()
  assign_view = view.browser.create_view(AssignmentsView)
  row = assign_view.selections.row(name: "Enterprise")
  selected_option = row.rate.widget.selected_option
...
def test_assign_storage_datastores(appliance, virtualcenter_provider)
  ...
  sel_datastore = cb.StorageAssign(assign_to: "Selected Datastores", selections: {"datastore" => {"Rate" => "Default"}})
...
def test_assign_storage_tagged_datastores(appliance, virtualcenter_provider)
  ...
  #   
  view = navigate_to(appliance.server, "Chargeback")
  tagged_datastore = cb.StorageAssign(assign_to: "Tagged Datastores", tag_category: "Location", selections: {"Chicago" => {"Rate" => "Default"}})
